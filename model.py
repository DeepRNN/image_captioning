...
class CaptionGenerator(BaseModel):
    ...
    def build_rnn(self):
        ...
        dec_w = weight('dec_weights', [dim_dec, num_words])  
        if params.init_dec_bias: 
            dec_b = bias('dec_biases', [num_words], init_val=self.word_table.word_freq)
        else:
            dec_b = bias('dec_biases', [num_words], init_val=0.0)
 
        # compute the mean context
        context_mean = tf.reduce_mean(contexts, 1)
       
        # initialize the LSTMs
        lstm = tf.nn.rnn_cell.LSTMCell(dim_hidden, initializer=tf.random_normal_initializer(stddev=0.05)) 
        lstm = tf.nn.rnn_cell.DropoutWrapper(lstm, keep_prob, keep_prob, keep_prob)

        temp = context_mean
        for i in range(params.num_init_layers):
            temp = fully_connected(temp, dim_hidden, 'init_lstm_fc1'+str(i))
            temp = batch_norm(temp, 'init_lstm_bn1'+str(i), is_train, use_batch_norm)
            temp = nonlinear(temp, 'tanh')
        memory = tf.identity(temp)
 
        temp = context_mean
        for i in range(params.num_init_layers):
            temp = fully_connected(temp, dim_hidden, 'init_lstm_fc2'+str(i))
            temp = batch_norm(temp, 'init_lstm_bn2'+str(i), is_train, use_batch_norm)
            temp = nonlinear(temp, 'tanh')
        ...
        for idx in range(max_sent_len):
            ...
            # Attention mechanism
            context_encode1 = fully_connected(context_flat, dim_ctx, 'att_fc11') 
            context_encode1 = batch_norm(context_encode1, 'att_bn11', is_train, use_batch_norm) 

            context_encode2 = fully_connected_no_bias(output, dim_ctx, 'att_fc12') 
            context_encode2 = batch_norm(context_encode2, 'att_bn12', is_train, use_batch_norm) 
            ...
            context_encode = dropout(context_encode, keep_prob, is_train)

            alpha = fully_connected_no_bias(context_encode, 1, 'att_fc2')                 
            alpha = batch_norm(alpha, 'att_bn2', is_train, use_batch_norm)
            ...
            logits1 = fully_connected(expanded_output, dim_dec, 'dec_fc')
            logits1 = nonlinear(logits1, 'tanh')
            logits1 = dropout(logits1, keep_prob, is_train)
        ...
        cross_entropy_loss = cross_entropy_loss / tf.reduce_sum(masks)
