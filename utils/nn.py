...
def weight(name, shape, init='he', range=0.15, stddev=0.05, init_val=None):
...
def bias(name, dim, init_val=0.0):
    ...
    var = tf.get_variable(name, dims, initializer = tf.constant_initializer(init_val))
    return var
...
def convolution(x, k_h, k_w, c_o, s_h, s_w, name, init_w='he', init_b=0, stddev=0.1, padding='SAME'):
    """ Apply a convolutional layer (with bias). """
    c_i = _get_shape(x)[-1]
    convolve = lambda i, k: tf.nn.conv2d(i, k, [1, s_h, s_w, 1], padding=padding)
    with tf.variable_scope(name) as scope:
        w = weight('weights', [k_h, k_w, c_i, c_o], init=init_w, stddev=stddev)
        z = convolve(x, w)
        b = bias('biases', c_o, init_b)
        z = tf.nn.bias_add(z, b)
    return z

def convolution_no_bias(x, k_h, k_w, c_o, s_h, s_w, name, init_w='he', stddev=0.1, padding='SAME'):
...
def fully_connected(x, output_size, name, init_w='he', init_b=0, stddev=0.05):
    """ Apply a fully-connected layer (with bias). """
    x_shape = _get_shape(x)
    input_dim = x_shape[-1]

    with tf.variable_scope(name) as scope:
        w = weight('weights', [input_dim, output_size], init=init_w, stddev=stddev)
        b = bias('biases', [output_size], init_b)
        z = tf.nn.xw_plus_b(x, w, b)
    return z

def fully_connected_no_bias(x, output_size, name, init_w='he', stddev=0.05):
...
def batch_norm(x, name, is_train, use_batch_norm=True):
    """ Apply a batch normalization layer if applicable. """
    if use_batch_norm:
